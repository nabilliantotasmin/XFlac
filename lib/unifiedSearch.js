...
/**
 * Search artist dari semua provider secara paralel + deduplicate.
 *
 * Provider yang support searchArtist():
 *   - Deezer  ✓ (searchArtistDeezer di server, tapi kita proxy via /api/search-artist)
 *   - Qobuz   ✓
 *   - Tidal   ✓
 *   - Amazon  ✓
 *   - Pandora ✗ (tidak support artist search)
 *
 * Karena searchArtist ada di server helpers, kita perlu mengaksesnya via instance
 * provider langsung (masing-masing provider punya method searchArtist).
 *
...
async function unifiedArtistSearch(query, limit = 8, timeoutMs = 10000) {
  // Providers yang support artist search
  const ARTIST_SEARCH_PROVIDERS = ['deezer', 'qobuz', 'tidal', 'amazon'];
  const activeProviders = PROVIDER_REGISTRY.filter(p =>
    ARTIST_SEARCH_PROVIDERS.includes(p.key)
  );
  ...
  const fetchPromises = activeProviders.map(async (meta) => {
    const instance = getProviderInstance(meta.key);
    if (!instance) {
      providerErrors[meta.key] = 'Provider not loaded';
      return;
    }
    if (typeof instance.searchArtist !== 'function') {
      providerErrors[meta.key] = 'searchArtist not implemented';
      return;
    }
    ...
    try {
      ...
      const searchPromise = instance.searchArtist(query, limit);
      const artists = await Promise.race([searchPromise, timeoutPromise]);

      if (Array.isArray(artists) && artists.length > 0) {
        allResults.push({ providerKey: meta.key, providerMeta: meta, artists });
        console.log(`[unified-artist] ${meta.key}: ${artists.length} results`);
      } else {
        console.log(`[unified-artist] ${meta.key}: no results`);
      }
    } catch (err) {
      providerErrors[meta.key] = err.message;
      console.warn(`[unified-artist] ${meta.key} failed: ${err.message}`);
    }
  });
