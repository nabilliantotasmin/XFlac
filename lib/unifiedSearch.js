...
/**
 * Normalisasi artist dari provider tertentu ke schema unified.
 * Output: { id, name, picture, albumsCount, fans, providers[] }
 */
function normalizeArtist(raw, providerKey) {
  return {
    id:          String(raw.id   || raw.name || ''),
    name:        raw.name        || 'Unknown',
    picture:     raw.picture     || raw.image || raw.cover || '',
    albumsCount: raw.albumsCount || raw.albums_count || 0,
    fans:        raw.fans        || raw.nb_fan || 0
  };
}

/**
 * Cek apakah dua artist adalah orang yang sama.
 * Kriteria: nama fuzzy-match (setelah strip aksen, lowercase).
 */
function isSameArtist(a, b) {
  const nameA = fuzzy(a.name);
  const nameB = fuzzy(b.name);
  ...
  if (nameA.length > 3 && nameB.includes(nameA)) return true;
  if (nameB.length > 3 && nameA.includes(nameB)) return true;

  // Levenshtein-lite: very short edit distance for common typos
  // Only apply for names ≥ 5 chars to avoid false positives
  if (nameA.length >= 5 && nameB.length >= 5) {
    ...
    // Allow 1 char difference per 8 chars of length
    const maxDiff = Math.floor(longer.length / 8);
    if (longer.startsWith(shorter) && longer.length - shorter.length <= maxDiff) return true;
  }
...
/**
 * Merge provider info ke artist yang sudah ada.
 * Prefer richer data (more fans, better picture).
 */
function mergeArtist(dest, src, providerKey) {
  // Prefer larger fan count
  if (src.fans > dest.fans) dest.fans = src.fans;
  // Prefer larger albumsCount
  if (src.albumsCount > dest.albumsCount) dest.albumsCount = src.albumsCount;
  // Prefer non-empty picture (first one wins, but override if dest has none)
  if (!dest.picture && src.picture) dest.picture = src.picture;

  // Add provider source tag
  if (!dest.providers.find(p => p.key === providerKey)) {
    dest.providers.push({ key: providerKey, artistId: src.id });
  }
}

// ─── Per-provider artist search helpers ──────────────────────────────────────
// Karena method searchArtist() tidak ada di semua provider instance,
// kita implementasikan langsung di sini menggunakan API masing-masing.

async function _searchArtistDeezer(query, limit) {
  const { request } = require('./utils');
  const url = `https://api.deezer.com/search/artist?q=${encodeURIComponent(query)}&limit=${limit}`;
  const res = await request(url, { timeout: 12000 });
  if (res.statusCode !== 200) throw new Error(`Deezer HTTP ${res.statusCode}`);
...
async function _searchArtistQobuz(query, limit) {
  const qobuz = getProviderInstance('qobuz');
  if (!qobuz) throw new Error('Qobuz provider not loaded');
  const data = await qobuz.apiGet('artist/search', { query, limit });
  return (data.artists?.items || []).map(a => ({
    id:          String(a.id),
    name:        a.name || 'Unknown',
    picture:     a.image?.large || a.image?.medium || a.image?.small || '',
    albumsCount: a.albums_count || 0,
    fans:        0
  }));
}

async function _searchArtistTidal(query, limit) {
  const { request } = require('./utils');
  const params = new URLSearchParams({
    countryCode: 'US', limit: String(limit), query
  });
  const url = `https://api.tidal.com/v1/search/artists?${params}`;
  const res = await request(url, {
    headers: { 'X-Tidal-Token': 'CzET4vdadNUFQ5JU', 'Accept': 'application/json' },
    timeout: 12000
  });
  if (res.statusCode !== 200) throw new Error(`Tidal HTTP ${res.statusCode}`);
  const data = JSON.parse(res.body);
  return (data.items || []).map(a => {
    const cover = a.picture
      ? `https://resources.tidal.com/images/${a.picture.replace(/-/g, '/')}/750x750.jpg`
      : '';
    return {
      id:          String(a.id),
      name:        a.name || 'Unknown',
...
async function _searchArtistAmazon(query, limit) {
  const amazon = getProviderInstance('amazon');
  if (!amazon) throw new Error('Amazon provider not loaded');
  if (typeof amazon.searchArtist !== 'function') throw new Error('Amazon searchArtist not available');
  const results = await amazon.searchArtist(query, limit);
...
// Map provider key → search function
const ARTIST_SEARCH_FNS = {
  deezer:  _searchArtistDeezer,
  qobuz:   _searchArtistQobuz,
  tidal:   _searchArtistTidal,
  amazon:  _searchArtistAmazon
  // pandora: tidak support artist search
};

/**
 * Search artist/publisher dari semua provider secara PARALEL + deduplicate.
 *
 * Provider yang digunakan: Deezer, Qobuz, Tidal, Amazon
 * (Pandora tidak punya artist search API)
 *
 * @param {string}  query      - Artist name to search
 * @param {number}  limit      - Max results per provider (default 8)
 * @param {number}  timeoutMs  - Per-provider timeout ms (default 10000)
 * @returns {Promise<{ artists: UnifiedArtist[], providerErrors: Object }>}
 */
async function unifiedArtistSearch(query, limit = 8, timeoutMs = 10000) {
  const providerErrors = {};
  const allResults = []; // [{ providerKey, artists }]

  // Parallel fetch semua provider sekaligus
  const fetchPromises = Object.entries(ARTIST_SEARCH_FNS).map(async ([provKey, searchFn]) => {
    try {
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`Timeout after ${timeoutMs}ms`)), timeoutMs)
      );
      const searchPromise = searchFn(query, limit);
      const artists = await Promise.race([searchPromise, timeoutPromise]);

      if (Array.isArray(artists) && artists.length > 0) {
        allResults.push({ providerKey: provKey, artists });
        console.log(`[unified-artist] ${provKey}: ${artists.length} results`);
      } else {
        console.log(`[unified-artist] ${provKey}: no results`);
      }
    } catch (err) {
      providerErrors[provKey] = err.message;
      console.warn(`[unified-artist] ${provKey} failed: ${err.message}`);
    }
  });
  ...
  for (const { providerKey, artists } of allResults) {
    for (const raw of artists) {
      const normalized = normalizeArtist(raw, providerKey);
      const existing = unified.find(u => isSameArtist(u, normalized));

      if (existing) {
        mergeArtist(existing, normalized, providerKey);
      } else {
        unified.push({
          ...normalized,
          providers: [{ key: providerKey, artistId: normalized.id }]
        });
      }
    }
  }

  // Sort: artists found across more providers first, then by fan count
  unified.sort((a, b) => {
  ...
  return { artists: unified, providerErrors };
}
