...
async function tagFile(filePath, trackData, onLog = () => {}, options = {}) {
  ...
  try {
    ...
    const [lyricsResult, mbTags, extraMeta] = await Promise.all([
      fetchLyrics({
      ...
      trackData.isrc ? mbClient.fetchMetadata(trackData.isrc).catch(err => {
        console.debug(`[tagger] MusicBrainz failed: ${err.message}`);
        return {};
      }) : Promise.resolve({}),
      fetchExtraMetadata(trackData.title, trackData.artist)
    ]);
