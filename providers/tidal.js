...
class TidalProvider {
   ...
   *  • MANIFEST (DASH/HLS segmented) with a direct_url → return it directly
   *  • MANIFEST without a direct_url → fallback to LOSSLESS, then HIGH
   *    (HI_RES tracks from some APIs come as DASH segments that need ffmpeg
   *     to reassemble; the browser cannot play them natively → we avoid them)
   *  • download() always reassembles segments via ffmpeg on disk
   * ─────────────────────────────────────────────────────────────────────────
   */
  async getStreamUrlOnly(trackId, quality = 'LOSSLESS') {
    await this.init();

    const rawId = String(trackId).startsWith('tidal_')
      ? trackId.replace('tidal_', '')
      : trackId;
    const id = this._parseTrackId(`https://listen.tidal.com/track/${rawId}`);

    // Helper: try to get a streamable (non-segmented) URL for a given quality
    const tryQuality = async (q) => {
      const dlUrl = await this._getDownloadUrl(id, q);

      if (!dlUrl.startsWith('MANIFEST:')) {
        // Direct URL — browser can play this natively
        const u = dlUrl.toLowerCase().split('?')[0];
        const format = (u.endsWith('.m4a') || u.endsWith('.mp4') || u.includes('aac')) ? 'm4a' : 'flac';
        return { url: dlUrl, format, encrypted: false };
      }

      // Parse manifest to check if it has a single direct_url (BTS JSON manifest)
      try {
        const result = parseManifest(dlUrl.slice(9));
        if (result.direct_url) {
          const fmt = (result.mime_type || '').toLowerCase().includes('flac') ? 'flac' : 'm4a';
          console.log(`[tidal] Manifest has direct_url for ${q} — using it`);
          return { url: result.direct_url, format: fmt, encrypted: false };
        }
      } catch {}

      // Segmented DASH manifest — browser cannot play; signal caller to try lower quality
      return null;
    };

    // Quality cascade: requested → LOSSLESS → HIGH
    const qualityOrder = [quality || 'LOSSLESS'];
    if (!qualityOrder.includes('LOSSLESS')) qualityOrder.push('LOSSLESS');
    if (!qualityOrder.includes('HIGH'))     qualityOrder.push('HIGH');

    let lastErr = new Error('Tidal: no streamable URL found');
    for (const q of qualityOrder) {
      try {
        const result = await tryQuality(q);
        if (result) {
          console.log(`[tidal] Stream URL resolved at quality=${q}`);
          return result;
        }
        console.warn(`[tidal] ${q} returned segmented DASH manifest — trying lower quality`);
      } catch (err) {
        console.warn(`[tidal] ${q} failed: ${err.message}`);
        lastErr = err;
      }
    }
  ...
  async download(track, quality, outputPath, onProgress) {
    // ─ DOWNLOAD PATH ─────────────────────────────────────────────────────────
    // Uses _getDownloadUrlWithFallback (same APIs) but writes to disk.
    // Handles segmented DASH manifests via _downloadFromManifest (ffmpeg remux).
    // ─────────────────────────────────────────────────────────────────────────
    await this.init();
